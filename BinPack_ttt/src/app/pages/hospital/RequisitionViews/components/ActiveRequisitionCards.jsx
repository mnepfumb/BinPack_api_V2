	...
	useEffect(() => {
		...
		const fetchRequisitionData = async () => {
			...
			try {
				...
				if (status === "success") {
					try {
						var requisition_url =  '/requisition/hospital?hospitalId='+users.company_id
		...
		<Grid container  sx={{ mb: '24px', mt: 2 }} sm={12}  style={{ gap: 15 }}>
			{requisitions.map((requisition, index) => (
				<Grid requisition xs={12} md={3} key={index}>
					<Link state={{ requisition: requisition }} to={{ pathname: '/netcare/hospital/view-requisitions-details', state: requisition }}>
